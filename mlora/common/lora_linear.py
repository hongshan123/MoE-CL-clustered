...
class LoraFunction(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        for lora_a, lora_b, lora_config, dropout, scaling in zip(args[::2],
            ...
            start_idx = lora_config.batch_start_idx_  # 这两个索引是干嘛的
            ...
            result.index_add_(0, lora_range[start_idx:end_idx], lora_data)  # 将lora的结果加到result中

            save_inputs += (lora_a, lora_b, drop_data)

        ctx.input_args = input_args  # ctx是干嘛的
...
class Linear(nn.Module):
    ...
    def __init__(self, base_layer: nn.Module, device: str):
        ...
        self.loras_ = nn.ModuleDict()  # 多个Lora增强
    ...
    def _efficient_impl(self, hidden_states: torch.Tensor, input_args: MultiLoraBatchData) -> torch.Tensor:
        ...
        for lora_config in input_args.lora_batch_data_config_:  # 多个lora是干嘛的？不同的批量数据使用不同的Lora
            adapter_name = "expert0"
        ...
        have_dora = any(lora.use_dora_ for lora in self.loras_.values())  # dora是啥？
