...
def generate(model: LLMModel,
    ...
    for cur_pos in range(min_tokens_len, total_len):
        attention_masks = torch.ones(tokens.size(0), cur_pos - prev_pos)
        ...
        outputs = model.forward(input_data)  # just use the first output
        for output in outputs:
            ...
            next_token = torch.where(
                input_text_mask[start_idx:end_idx, cur_pos], tokens[start_idx:end_idx, cur_pos], next_token
            )
            tokens[start_idx:end_idx, cur_pos] = next_token

            stop_reached[start_idx:end_idx] |= (~input_text_mask[:, cur_pos]) & (next_token == tokenizer.eos_id_)
