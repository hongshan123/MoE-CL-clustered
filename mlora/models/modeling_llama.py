...
class LlamaMLP(nn.Module):
    def __init__(self, w1: nn.Module, w2: nn.Module, w3: nn.Module, args: LlamaConfig) -> None:
        super().__init__()
        # feed forward
        self.w1_: Linear = Linear(w1, args.device_)  # gate network
        self.w2_: Linear = Linear(w2, args.device_)  # down network
        self.w3_: Linear = Linear(w3, args.device_)  # up network
        self.act_ = ACT2FN[args.hidden_act_]

    def lora_state_dict(self) -> Dict[str, nn.Module]:
        return {
            "w1_proj": self.w1_,
            "w2_proj": self.w2_,
            "w3_proj": self.w3_,
    ...
    def _mixlora_forward(self, act_fn, expert_idxs, hidden_states, input_dtype):  # hidden_states: batch_size * seq_len, dim
        ...
        for e_id in expert_idxs:  # traversal all expert, including shared expert and specific expert
            ...
            if lora_name in self.w3_.loras_:
                w3 = self.w3_.loras_[lora_name].forward(common_w3, hidden_states)
            else:
                w3 = common_w3

            act_result = act_fn(w1) * w3
            if lora_name in self.w2_.loras_:
                final_expert_states.append(self.w2_.loras_[lora_name].forward(self.w2_.base_layer_.forward(act_result), act_result))
            else:
...
class LlamaDecoderLayer(nn.Module):
    ...
    def lora_state_dict(self) -> Dict[str, nn.Module]:
        linear_layers = self.self_attn_.lora_state_dict()
        linear_layers.update(self.mlp_.lora_state_dict())
...
class LlamaEmbedding(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        data = F.embedding(tokens, self.token_embedding,
...
class LlamaSequentialWrapper(nn.Module):
    ...
    def forward(self, input: Tuple) -> Tuple:
        ...
        if module_name == "LlamaEmbedding":
            ...
            if len(outputs) > 1:  # 不返回就没有分类损失了
                self.classifier_probs_ = outputs[1:]
...
class LlamaForCausalLM(LLMForCausalLM):
    ...
    def from_pretrained(llm_model: modeling_llama.LlamaForCausalLM,
        ...
        if llm_config.model_type == "hunyuan":  # 32个query头，8个key-value头，每个头的隐藏层维度为128
            for idx, layer in enumerate(llm_model.model.layers):  # copy weights from llm_model
                decoder = LlamaDecoderLayer(idx)

                # 从 qkv_proj 中提取出 query、key 和 value 的权重
                qkv_weight = layer.self_attn.qkv_proj.weight
                q_weight = qkv_weight[:4096, :]
                k_weight = qkv_weight[4096:5120, :]
                v_weight = qkv_weight[5120:, :]

                q_proj = nn.Linear(4096, 4096, bias=None, dtype=torch.bfloat16)
                k_proj = nn.Linear(4096, 1024, bias=None, dtype=torch.bfloat16)
                v_proj = nn.Linear(4096, 1024, bias=None, dtype=torch.bfloat16)
                ...
                # 从 gate_and_up_proj 中提取出 gate 和 up 的权重
                gate_and_up_weight = layer.mlp.gate_and_up_proj.weight
                gate_proj_weight, up_proj_weight = gate_and_up_weight.chunk(2, dim=0)

                gate_proj = nn.Linear(4096, 14336, bias=None, dtype=torch.bfloat16)
                up_proj = nn.Linear(4096, 14336, bias=None, dtype=torch.bfloat16)

                with torch.no_grad():
                    gate_proj.weight.copy_(gate_proj_weight)
                    up_proj.weight.copy_(up_proj_weight)
